inflated.response = function(object,
                             predictors,
                             select.columns = NULL,
                             label = NULL,
                             len = 50,
        ...
        if (is.null(select.columns))
                select.columns = 1:ncol(predictors)
        ...
        for (i in select.columns)
        {
                ...
                if (method == "stat3") {
                        summaries.j = as.matrix(summaries[c(1, 4, 6), -i], ncol = (ncol(predictors) -
                                                                                           1))
                ...
                if (comb < lhsample) {
                        ...
                        for (m in 1:ncol(dummy.j))
                                mat[[m]] = 1:nc
                        mat = expand.grid(mat)
                } else
                        mat = round(qunif(
                                randomLHS(lhsample, ncol(dummy.j)),
                                1,
                                nrow(summaries.j)
                        ), 0)
                if (is.null(label))
                ...
                for (r in 1:nrow(mat))
                {
                        ...
                        #                 type = "response"
                        #         ) # when using brt code from Elith et al. (2008) JAnimEcol
                        # else if (is(object, "glm"))
                        #         curves <- predict(object, dummy, type = "response")
                        # else if (is(object, "randomForest"))
                        #         curves <- predict(object, dummy)
                        # else if (is(object, "tree"))
                        #         curves <- predict(object, dummy)
                        # else if (is(object, "list"))
                        #         curves <-
                        #         mars.predict(object, dummy)$prediction[[1]]   #when using mars code from Elith & Leathwick (2007) Div Distr
                        # else if (is(object, "fda"))
                        #         curves <- predict(object, dummy, type = "post")[, 2]
                        # else if (is(object, "nnet"))
                        #         curves <- predict(object, dummy, type = "raw")
                        # else {
                        #         print("SDM class unknown")
                        #         break
                        # }
                        
                        if (isTRUE(ensemble.mode)) {
                                c1 <- predict(object[[1]], dummy, type = "response")
                                c2 <- predict(object[[2]], dummy, type = "response")
                                c3 <- predict(object[[3]], dummy, type = "response")
                                c4 <- predict(object[[4]], dummy, type = "response")
                                curves <- (c1 + c2 + c3 + c4)/4
                        } else{
                                curves <- predict(object, dummy, type = "response")
                        }
                ...
                if (overlay.mean == T)
                {
                        ...
                        #         curves <- predict(object, dummy)
                        # else if (is(object, "list"))
                        #         curves <- mars.predict(object, dummy)$prediction[[1]]
                        # else if (is(object, "fda"))
                        #         curves <- predict(object, dummy, type = "post")[, 2]
                        # else if (is(object, "nnet"))
                        #         curves <- predict(object, dummy, type = "raw")
