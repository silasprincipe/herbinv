...
pseudoabGen <- function(species, nb.rep = 10, pa.nb = 2){
        ...
        cat("Generating PA for", species, "Number of PA points:",
            pa.nb, " Number of PA replicates:", nb.rep, "\n")
        ...
        for (i in 1:nb.rep) {
                ...
                pnf<- table(pa.table$dsp[pa.table$PA1 == T], useNA = "ifany")[1]
                anf<- table(pa.table$dsp[pa.table$PA1 == T], useNA = "ifany")[2]
                
                if (pnf != (anf/pa.nb)) {
                   stop("Wrong number of absences produced.")     
                }
                
                # Check if the procedure worked and don't produced any duplicated
                if (any(duplicated(pa.table[, 1:2]) == T)){
                        stop("Duplicated values.")
                }
                
                # Write final file with full table
                write.csv(pa.table,
                          paste0(path, species, "_pa_table_", i,".csv"),
                          row.names = F)
                ...
                write.csv(pts,
                          paste0(path, species, "_pts_", i,".csv"),
                          row.names = F)
                write.csv(dsp,
                          paste0(path, species, "_dsp_", i,".csv"),
                          row.names = F)
                write.csv(set,
                          paste0(path, species, "_set_", i,".csv"),
                          row.names = F)
        }
