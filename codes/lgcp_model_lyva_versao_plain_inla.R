...
forms[[5]] <- update(forms[[1]], ~ . + chl(env.e, model = "linear"))

forms[[6]] <- update(forms[[1]], ~ . + ph(env.e, model = "linear") 
                     + chl(env.e, model = "linear"))

# Include models with SPDE
forms[[7]] <- update(forms[[1]], ~ . + spatial(coordinates, model = b.model,
...
forms[[3]] <- as.formula(paste(
    "y ~ 0 + b0 + sal + ph + f(x, model = d1spde) +",
    "f(s, model = b.model)"
))

forms[[4]] <- as.formula(paste(
    "y ~ 0 + b0 + sal + ph + chl + f(x, model = d1spde) +",
    "f(s, model = b.model)"
))
...
locations <- rbind(ips@coords,
                   pts@coords)
rownames(locations) <- 1:nrow(locations)

mesh.loc <- SpatialPoints(mesh$loc[,1:2], proj4string = crs(env.e))
mesh.loc <- SpatialPoints(cbind(locations[,1], locations[,2]), proj4string = crs(env.e))


sp.data <- over(mesh.loc, env.e)
sp.data[is.na(sp.data$ph),] <- 0

## Projection matrix
A.i <- inla.spde.make.A(mesh, loc = locations)

A.sst <- inla.spde.make.A(d1mesh, sp.data$sst)

spde1.idx <- inla.spde.make.index("x", n.spde = d1spde$n.spde)
...
for (i in 1:length(forms)) {
        ...
        stk <- inla.stack(data=list(y=c(rep(0, length(ips)),
                                        rep(1, length(pts))),
                                    e=c(ips$weight, rep(0, length(pts)))), 
                          effects=list(s= 1:mesh$n, # spatial random effect
                                       data.frame(b0=rep(1, nrow(data.sp)),
                                                  data.sp),
                                       spde1.idx), #covariates
                          A=list(A.i, 1, A.sst), # projection matrix
        ...
        m[[i]] <- inla(forms[[i]],
                       ...
                       control.predictor=list(A=inla.stack.A(stk),
                                              compute = TRUE,
                                              link = 1),
                       family="poisson", E = inla.stack.data(stk)$e,
        ...
        m.lambda[[i]] <- sum(fit.pred * c(ips$weight, rep(0, length(pts))))
        
        cat("Number of points:", length(pts), "--- Lambda:\n")
        cat("\nMean lambda:", m.lambda[[i]], "\n")
...
sel.model <- c(4, 8) # we also project the one with the spatial component
...
pred.comp <- predict(m[[sel.model[1]]], data = df,
                     formula = ~ list(
                             ...
                             sal = sal,
                             ph = ph,
                             combl = sst + sal + ph + Intercept,
                             combl_exp = exp(sst + sal + ph + Intercept)),
