...
pts <- read.csv(paste0("data/", species, "/", species, "_final.csv"))[,2:3]
...
forms[[7]] <- update(forms[[4]], ~ . + spatial(coordinates, model = spde,
                                               mapper = bru_mapper(mesh)))

forms[[8]] <- update(forms[[5]], ~ . + spatial(coordinates, model = spde,
                                               mapper = bru_mapper(mesh)))

forms[[9]] <- update(forms[[6]], ~ . + spatial(coordinates, model = spde,
                                               mapper = bru_mapper(mesh)))


Aconstr <- matrix(nrow = 1, ncol = mesh$n)

meshloc <- SpatialPoints(mesh$loc[,1:2], proj4string = crs(env.e))

Aconstr[1,] <- (colSums(inla.spde.make.A(mesh, ips)) * over(meshloc, env.e[,"sst"]))[,1]
Aconstr[is.na(Aconstr)] <- 0
#Aconstr[2,] <- (colSums(inla.spde.make.A(mesh, ips)) * over(meshloc, env.e[,"sst"]))[,1]

forms[[7]] <- coordinates ~ 
    Intercept(1, mean.linear = log(length(pts)/gArea(starea)), prec.linear = 1) + 
    sal(env.e, model = "linear", mean.linear = 0, prec.linear = 1) +
   # dist(env.e, model = "linear", mean.linear = 0, prec.linear = 1)+
   sst(env.e, model = d1spde.st) +
    spatial(coordinates, model = spde, extraconstr = list(A = Aconstr, e = 0))
...
pred.comp <- predict(m[[4]], data = df,
                 formula = ~ list(
                         ...
                         ph = ph,
                         #spatial = spatial,
                         int = exp(sst + sal + ph + dist +
                                      # spatial +
                                       Intercept)
                 ))

ggplot()+
        gg(pred.comp$int, aes(fill = mean))+
    sca(pred.comp$int$mean)+# change here according to layer
    #gg(pts, size = .5)+
    coord_equal()
        #coord_equal(xlim = c(-2400,0), ylim = c(2500, 4800))
