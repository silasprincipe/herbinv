...
lik2 <- like(data =  pts.c,
             ...
             formula = coordinates ~ spdeCopy + sst + ph + sal + lgcpIntercept)
...
forms[[3]] <- coordinates ~ spdeCopy + ph + sst + sal + lgcpIntercept
forms[[4]] <- coordinates ~ spdeCopy + chl + sst + sal + lgcpIntercept
forms[[5]] <- coordinates ~ spdeCopy + bath + ph + sst + sal + lgcpIntercept
forms[[6]] <- coordinates ~ spdeCopy + bath + chl + sst + sal + lgcpIntercept
forms[[7]] <- coordinates ~ spdeCopy + chl + ph + sst + sal + lgcpIntercept
forms[[8]] <- coordinates ~ spdeCopy + bath + ph + chl + sst + sal + lgcpIntercept
forms[[9]] <- coordinates ~ spde + sst + sal + lgcpIntercept

lamb <- list()
lamb[[1]] <- "spdeCopy + sst + sal + lgcpIntercept"
lamb[[2]] <- "spdeCopy + bath + sst + sal + lgcpIntercept"
lamb[[3]] <- "spdeCopy + ph + sst + sal + lgcpIntercept"
lamb[[4]] <- "spdeCopy + chl + sst + sal + lgcpIntercept"
lamb[[5]] <- "spdeCopy + bath + ph + sst + sal + lgcpIntercept"
lamb[[6]] <- "spdeCopy + bath + chl + sst + sal + lgcpIntercept"
lamb[[7]] <- "spdeCopy + chl + ph + sst + sal + lgcpIntercept"
lamb[[8]] <- "spdeCopy + bath + ph + chl + sst + sal + lgcpIntercept"
lamb[[9]] <- "spde + sst + sal + lgcpIntercept"
...
deltaIC(m[[1]], m[[2]], m[[3]], m[[4]], m[[5]],
        m[[6]], m[[7]], m[[8]], m[[9]], criterion = "DIC")

exp((3115.417 - 3609.665)/2) 
...
for (i in 1:9){
        # pred[[i]] <- predict(m[[i]],
        #                     data = df,
        #                     formula = as.formula(paste("~ exp(", lamb[[i]], ")")))
        print(ggplot() +
                gg(pred[[i]], aes(fill = mean)) + sc(pred[[i]]$mean) + coord_equal(
                        ylim = c(-4200,0)
                )+
                      geom_point(data = data.frame(pts.c@coords),
                                  aes(x=x, y=y), alpha = .1))
}
