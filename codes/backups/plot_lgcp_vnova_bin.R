...
curr <- raster(paste0("results/", sp, "/predictions/", sp, "_mean_m4_current.tif"))
ssp1 <- raster(paste0("results/", sp, "/predictions/", sp, "_mean_m4_ssp1.tif"))
ssp2 <- raster(paste0("results/", sp, "/predictions/", sp, "_mean_m4_ssp2.tif"))
ssp3 <- raster(paste0("results/", sp, "/predictions/", sp, "_mean_m4_ssp3.tif"))
...
ssp1 <- calc(ssp1, con.th)
ssp2 <- calc(ssp2, con.th)
ssp3 <- calc(ssp3, con.th)
...
sca <- scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, "GnBu"),
                            limits = c(0,1))

sca <- scale_fill_gradientn(colours = rev(RColorBrewer::brewer.pal(11, "RdGy")),
                            limits = c(0,1))

sca <- scale_fill_gradientn(colours = c("#023047", "#219ebc", "#8ecae6", "#ffb703", "#fb8500"),
                            limits = c(0,1))

sca <- scale_fill_gradientn(colours = c("#6C4E67", RColorBrewer::brewer.pal(9, "YlGnBu")),
                            limits = c(0,1))

sca <- scale_fill_gradientn(colours = c("#B8AA94", "#DAD4A2", RColorBrewer::brewer.pal(9, "YlGnBu")),
                            limits = c(0,1))

sca <- scale_fill_gradientn(colours = rev(RColorBrewer::brewer.pal(9, "YlGnBu")),
                            limits = c(0,1))

sca <- scale_fill_gradientn(colours = rev(RColorBrewer::brewer.pal(9, "BuPu")),
                            limits = c(0,1))


sca <- scale_fill_gradientn(colours = rev(c("#ff8818", "#F59E42", "#F5B776", "#F5E876", "#CAEFF0",
                                        "#34A0A4", "#168AAD", "#1A759F", "#1E6091", "#184E77")),
                            limits = c(0,1))
#"#021B44", 
sca <- scale_fill_gradientn(colours = c("#00296A", "#074CBB", "#4174C1", "#7B9CC6",
                                            "#E7E4C8", "#DEB969", "#DAA43A", "#D89922", "#D58E0A"),
                            limits = c(0,1))

sca <- scale_fill_stepsn(breaks = seq(0,1, length.out = 8),
        colours = c("#00296A", "#074CBB", "#4174C1", "#7B9CC6",
                                     "#E7E4C8", "#DEB969", "#DAA43A", "#D89922", "#D58E0A"),
                         limits = c(0,1))

sca <- scale_fill_stepsn(breaks = seq(0,1, length.out = 8),
                         colours = c("#00296A", "#074CBB", "#4174C1", "#7B9CC6",
                                     "#E7E4C8", "#DFA06A", "#DB7E3B", "#D96D23", "#D65C0B"),
                         limits = c(0,1))
...
ggsave(paste0("figures/", sp, "_lgcp_current_m4.jpg"), pcf,
...
rcp.scale <-
        scale_fill_manual(
                values = c(
                        #c("#BA1F43", "#D64B69", "#E696A6", "#DCA2EB", "#AA5EBD", "#8823A1")
                        c("#4E1580", "#723E9C", "#B18ECC", "#ADDBCB", "#5CB899", "#238061")
                ),
                limits = c("HD", "MD", "LD", "LI",
                           "MI", "HI"),
                breaks = c("HD", "MD", "LD", "LI",
                           "MI", "HI"),
                labels = c("High decrease", "Moderate decrease", "Low decrease",
...
rcp.scale.b <- scale_fill_stepsn(
        breaks = c(-50, 0, 50),
        limits = c(-100,100),
        colours = c("#1E0433", "#723E9C", "#5CB899", "#0B4F3A"),
        #labels = c("Mod. decrease", "", "Mod. increase"),
        #guide = step.guide
)
