...
# After judgement, for this species we chose: MEAN SST
...
pred.comp <- predict(m[[5]], data = df,
                 formula = ~ list(
                         ...
                         sal = sal,
                         chl = chl,
                         int = exp(sst + sal + chl + Intercept)
                 ))

ggplot()+
        gg(pred.comp$int)+ # change here according to layer
        #gg(pts, size = .5, color = "green")+
        coord_equal()

# We select the model 5 as the best one, considering WAIC and exploratory analysis
sel.model <- 5
...
# save
sapply(names(pred.comp), save.rast, model = 5)

# Predict model with spatial component
pred.comp <- predict(m[[9]], data = df,
                     formula = ~ list(
                             coldm = coldm,
                             sal = sal,
                             chl = chl,
...
# save
sapply(names(pred.comp), save.rast, model = 9)
