...
forms[[1]] <- update(forms[[1]], ~ . + sst(env.e, model = d1spde.st))
...
forms[[1]] <- update(base.f, ~ . + spatial(coordinates, model = spde))
...
ggplot()+
        gg(pred.comp$dist, aes(fill = mean))+ # change here according to layer
        coord_equal()
