...
crs(pts) <- crs("+proj=longlat +ellps=WGS84 +nodefs")
...
starea.simp <- gSimplify(gBuffer(starea, width=0.3*111), tol=20)
...
conv.t <- inla.nonconvex.hull(ipoints(starea, mesh), convex = 6*dgk)

# Creates mesh and plot
mesh <- inla.mesh.2d(boundary = list(
    starea,
    conv.t
),
...
forms[[1]] <- update(base.f, ~ . + spatial(main = coordinates, model = b.model,
                                               mapper = bru_mapper(mesh)))

forms[[3]] <- update(forms[[1]], ~ . + spatial(main = coordinates, model = spde))
...
pred.comp.sp <- predict(m[[3]], data = df,
                     formula = ~ list(
                         # Change those according to mode and
                         # combinations
                         # warmm = warmm,
...
p1 <- ggplot()+
        gg(pred.comp.sp$int)+ # change here according to layer
    scale_fill_viridis()+
    #gg(pts, color = 'white')+
        coord_equal()
...
pred.spat <- predict(m[[2]], data = df,
                        formula = ~ spatial)

ggplot()+
    gg(pred.spat)+ # change here according to layer
    scale_fill_viridis()+
    #gg(pts, color = 'white')+
    coord_equal()
