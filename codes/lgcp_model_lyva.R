...
pred.comp <- predict(m[[4]], data = df,
                 formula = ~ list(
                         ...
                         int = exp(sst + sal + ph + dist + spatial + Intercept),
                         # Linear predictor scale
                         lin = sst + sal + ph + dist + spatial + Intercept,
                         # Contrast models (excluding spatial component)
                         int_cont = exp(sst + sal + ph + dist + Intercept),
                         lin_cont = sst + sal + ph + dist + Intercept
                 ))
...
for (i in 1:2) {
   ...
   # Get formula
    pred.f <- list(
      as.formula("~exp(sst + sal + dist + spatial + ph + Intercept)"),
      as.formula("~exp(sst + sal + dist + ph + Intercept)")
    )[[i]]
