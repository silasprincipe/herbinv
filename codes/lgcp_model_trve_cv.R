...
# Get an integration object to hold CV results
ips.cv <- ips
ips.cv[1:nrow(ips.cv),"cv_res"] <- NA
...
for (i in 1:5) {
  ...
  pred.ips <- predict(cv.m, data = ips,
                      formula = ~ weight * exp(sst + sal + dist + spatial + Intercept))
  
  pred.ips$cv.pred <- 1 - exp(-pred.ips$mean)
  
  cv.roc[[i]] <- roc(pa.pts$pa ~ pred.ips$cv.pred)
  
  cv.roc[[i]]$cor <- cor.test(pa.pts$pa, pred.ips$cv.pred)
    
  # Get the distance from presence points and integration points
  # predpts <- pts[blocks == i,]
  # distpts <- pointDistance(ips, predpts)
  # 
  # # Get the closest points
  # clpts <- apply(distpts, 2, function(x){which(x == min(x))})
  # 
  # for (k in 1:length(clpts)) {
  #   ips.cv$cv_res[clpts[k]] <- ifelse(is.na(ips.cv$cv_res[clpts[k]]),
  #                                     pred.ips$full$mean[clpts[k]],
  #                                     mean(ips.cv$cv_res[clpts[k]], pred.ips$full$mean[clpts[k]]))
  # }
...
pred.ips.full <- predict(m[[sel.model]], data = ips,
                    formula = ~ list(
                      full = weight * exp(sst + sal + dist + spatial + Intercept),
                      cont = weight * exp(sst + sal + dist + Intercept)
                    ))

pa.pts$pred <- 1 - exp(-pred.ips$full$mean)
ips.cv$cv.pred <- 1 - exp(-ips.cv$cv_res) 

model.roc <- roc(pa.pts$pa~pa.pts$pred)
model.roc.cv <- roc(pa.pts$pa~ips.cv$cv.pred)
...
lines(1 - model.roc.cv$specificities, model.roc.cv$sensitivities)
legend("bottomright", legend = c("Fit", "CV"), col = c("blue", "black"),
       lty = c(1, 1))


### TESTE
pa.pts$pred <- 1 - exp(-pred.ips.full$cont$mean)
pred.ips$full$cv.pred <- 1 - exp(-pred.ips$cont$mean) 


model.roc <- roc(pa.pts$pa~pa.pts$pred)
model.roc.cv <- roc(pa.pts$pa~pred.ips$full$cv.pred)
...
### TESTE MEAN
pa.pts$pred <- 1 - exp(-pred.ips.full$full$mean)
model.roc <- roc(pa.pts$pa~pa.pts$pred)
...
mean(unlist(lapply(cv.roc, function(x){as.numeric(x$cor$estimate)})))
