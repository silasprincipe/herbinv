...
env <- var.load(layers = lays, bioclim = biocl, bath = "2_100")

r12 <- var.load(folder = "proj_layers/ssp126/", layers = lays,
                bioclim = biocl, bioclimfut = "ssp126", bath = "2_100")
r24 <- var.load(folder = "proj_layers/ssp245/", layers = lays,
                bioclim = biocl, bioclimfut = "ssp245", bath = "2_100")
r37 <- var.load(folder = "proj_layers/ssp370/", layers = lays,
                bioclim = biocl, bioclimfut = "ssp370", bath = "2_100")
...
names(env) <- c("ph", "chl", "sal", "sst", "bath", "coldm", "warmm")
...
mesh1 <- inla.mesh.2d(boundary = starea,
                     max.edge = c(1.4, 5)*dgk,
                     cutoff = 0.32*dgk, crs = crs(env),
                     offset = c(0.1,10)*dgk)

ggplot()+gg(mesh1)+coord_fixed()+spatt;mesh1$n

ips <- ipoints(starea, mesh1)

ggplot()+gg(mesh1)+gg(ips, color = "purple", size = 1)+coord_fixed(xlim = c(-2500, 1500), ylim = c(300, 3500))+spatt
ggplot()+gg(mesh1)+gg(ips, color = "purple", size = 1)+coord_fixed(xlim = c(2000, 4200), ylim = c(-3000, -500))+spatt
