...
layers <- data.frame(
        bior = c("ph", "chlomax", "chlomean", "dissoxmean", "dissoxmin",
                 "salinitymean", "salinitymin", "silicatemax", "silicatemean",
                 "tempmax", "tempmean"),
        cmip = c("phos", "chlos", "chlos", "o2os", "o2os", "sos", "sos",
                 "sios", "sios", "tos", "tos"),
        var = c("mean", "max", "mean", "mean", "min", "mean", "min", "max",
                "mean", "max", "mean")
)
...
for (k in 1:4) {
        ...
        for (i in 1:nrow(layers)) {
                ...
                if (layers[i, 2] == "chlos") {
                        cmip.f <- cmip.f * 1000000
                }
                
                if (layers[i, 2] == "o2os" | layers[i, 2] == "sios") {
                        cmip.f <- cmip.f * 1000
                }
