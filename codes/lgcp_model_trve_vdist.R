...
pred.comp <- predict(m[[5]], data = df,
                 formula = ~ list(
                         ...
                         pho = pho,
                         #spatial = spatial,
                         int = exp(coldm + sal + dist + chl + pho +
                                      # spatial +
                                       Intercept)
                 ))

ggplot()+
        gg(pred.comp$int, aes(fill = mean))+ # change here according to layer
        coord_equal()

# We select the model 4 as the best one, considering WAIC and exploratory analysis
sel.model <- c(5, 8) # we also project the one with the spatial component
...
pred.comp <- predict(m[[sel.model[1]]], data = df,
                     formula = ~ list(
                             ...
                             chl = chl,
                             pho = pho,
                             combl = coldm + sal + dist + chl + pho + Intercept,
                             combl_exp = exp(coldm + sal + dist + chl + pho + Intercept)),
